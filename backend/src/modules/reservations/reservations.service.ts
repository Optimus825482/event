...
import { findTableLabel } from "./table-location.helper";
...
export class ReservationsService {
  ...
  private async sendTicketEmailAsync(
    ...
    try {
      ...
      // Masa etiketini bul
      const tableLabel = findTableLabel(event.venueLayout, reservation.tableId);
  ...
  /**
   * Geçmiş etkinliklerdeki PENDING/CONFIRMED rezervasyonları NO_SHOW olarak işaretle
   */
  async markNoShows(eventId: string): Promise<{ updated: number }> {
