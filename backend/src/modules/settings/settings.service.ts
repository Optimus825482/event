...
export class SettingsService implements OnModuleInit {
  ...
  async createStaffColor(data: { name: string; color: string }): Promise<StaffColor> {
  ...
  private async ensureDefaultTableTypes() {
    ...
    if (count === 0) {
      const defaultTypes = [
        { name: 'Standart', capacity: 8, color: '#3b82f6', shape: 'round', minSpacing: 50 },
        { name: 'VIP', capacity: 12, color: '#eab308', shape: 'round', minSpacing: 60 },
        { name: 'Premium', capacity: 10, color: '#8b5cf6', shape: 'round', minSpacing: 55 },
        { name: 'Loca', capacity: 15, color: '#ef4444', shape: 'rectangle', minSpacing: 80 },
      ];
