...
// Masa rezervasyon bilgisi (canvas için)
...
interface CanvasState {
  ...
  // Rezervasyon entegrasyonu (Requirements 8.1, 8.3, 8.4)
  reservations: Reservation[];
  tableReservations: Record<string, TableReservationInfo>; // tableId -> reservation info
  setReservations: (reservations: Reservation[]) => void;
  updateTableReservation: (
    tableId: string,
    info: TableReservationInfo | null
  ) => void;
  getTableReservation: (tableId: string) => TableReservationInfo | null;
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
  ...
  placeAllPendingTables: () => {
    ...
    set((state) => {
      ...
      const vipType = tableTypes.find((t) =>
      ...
      if (stdType && stdCount > 0) {
        ...
        for (let row = 0; row < 15 && placed < stdCount; row++) {
          ...
          const inStageEndZone =
            y >= stageExtBottom && y < stageFrontBottom + 10;
          const inSysKontrolZone =
            y >= sysKontrolY - 25 && y < sysKontrolY + 45;
  ...
  setReservations: (reservations) => {
    // Rezervasyonları tableId bazlı map'e dönüştür
    const tableReservations: Record<string, TableReservationInfo> = {};
    reservations.forEach((res) => {
      // Sadece aktif rezervasyonları göster (cancelled ve no_show hariç)
      if (res.status !== "cancelled" && res.status !== "no_show") {
        tableReservations[res.tableId] = {
          reservationId: res.id,
          customerName: res.customer?.fullName || "Misafir",
          guestCount: res.guestCount,
          status: res.status,
        };
      }
    });
    set({ reservations, tableReservations });
  },

  updateTableReservation: (tableId, info) =>
    set((state) => {
      const newTableReservations = { ...state.tableReservations };
      if (info) {
        newTableReservations[tableId] = info;
      } else {
        delete newTableReservations[tableId];
      }
      return { tableReservations: newTableReservations };
    }),

  getTableReservation: (tableId) => {
    return get().tableReservations[tableId] || null;
  },

  clearReservations: () => set({ reservations: [], tableReservations: {} }),

  // Real-time check-in güncellemesi (Requirement 8.4)
  handleCheckInUpdate: (
    tableId: string,
    status: "checked_in" | "cancelled" | "no_show"
  ) =>
    set((state) => {
      const currentReservation = state.tableReservations[tableId];
      if (!currentReservation) return state;

      if (status === "cancelled" || status === "no_show") {
        // İptal veya no-show durumunda rezervasyonu kaldır
        const newTableReservations = { ...state.tableReservations };
        delete newTableReservations[tableId];
        return { tableReservations: newTableReservations };
      }

      // Check-in durumunu güncelle
      return {
        tableReservations: {
