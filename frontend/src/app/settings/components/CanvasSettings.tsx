  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Yerleşim planı ve ızgara ayarları</p>
      </div>

      <div className="space-y-4">
        <div>
          <label className="block text-sm text-slate-400 mb-2">Izgara Boyutu (px)</label>
          <input
            type="number"
            value={localSettings.defaultGridSize}
            onChange={(e) =>
              setLocalSettings({ ...localSettings, defaultGridSize: Number(e.target.value) })
            }
            min={10}
            max={100}
            step={5}
        ...
        <div className="space-y-3">
          <label className="flex items-center justify-between p-4 bg-slate-700 rounded-lg cursor-pointer">
            <div>
              ...
              <p className="text-sm text-slate-400">Masalar otomatik ızgaraya hizalansın</p>
            </div>
            <input
              type="checkbox"
          ...
          <label className="flex items-center justify-between p-4 bg-slate-700 rounded-lg cursor-pointer">
            <div>
              ...
              <p className="text-sm text-slate-400">Canvas açıldığında ızgara görünsün</p>
            </div>
            <input
              type="checkbox"
              checked={localSettings.showGridByDefault}
              onChange={(e) =>
                setLocalSettings({ ...localSettings, showGridByDefault: e.target.checked })
              }
              className="w-5 h-5 rounded bg-slate-600 border-slate-500"
            />
          </label>
