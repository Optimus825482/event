...
export function TableTypesSettings() {
  const { tableTypes, fetchTableTypes, addTableType, updateTableType, deleteTableType } =
    useSettingsStore();
  const [editingId, setEditingId] = useState<string | null>(null);
  const [showAddForm, setShowAddForm] = useState(false);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState({ name: '', capacity: 8, color: getRandomDefaultColor() });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bu masa türünü silmek istediğinize emin misiniz?')) return;
    try {
  ...
  const startEdit = (type: TableTypeConfig) => {
    ...
    setFormData({ name: type.name, capacity: type.capacity, color: type.color });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Masa kategorileri ve kapasiteleri</p>
        </div>
        <button
          onClick={() => {
            setFormData({ name: '', capacity: 8, color: getRandomDefaultColor() });
        ...
        <div className="p-4 bg-slate-700 rounded-lg border border-blue-500">
          ...
          <div className="flex items-end gap-4">
            <div className="flex-1">
              <label className="block text-sm text-slate-400 mb-1">Masa Tipi Adı</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div className="w-28">
              <label className="block text-sm text-slate-400 mb-1">Kişi Sayısı</label>
              <input
                type="number"
                min={1}
      ...
      <div className="space-y-2">
        {[...tableTypes].sort((a, b) => a.name.localeCompare(b.name, 'tr')).map((type) => {
          const isLoca = type.name.toLowerCase() === 'loca';
          
          return (
            <div key={type.id} className={`flex items-center gap-4 p-4 bg-slate-700 rounded-lg ${isLoca ? 'opacity-60' : ''}`}>
              {editingId === type.id && !isLoca ? (
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditingId(null)} className="p-2 bg-slate-600 rounded-lg">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </>
                ...
                <>
                  ...
                  <div className="flex-1">
                    <p className="font-medium">
                      {type.name}
                      {isLoca && <span className="ml-2 text-xs text-slate-400">(Sabit)</span>}
                    </p>
                  </div>
                  <div className="w-28 text-slate-400">{type.capacity} kişilik</div>
                  {!isLoca && (
                    <div className="flex items-center gap-2">
                      <button onClick={() => startEdit(type)} className="p-2 bg-slate-600 rounded-lg">
