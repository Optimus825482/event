...
function QuickAddPanel({ tableTypes }: { tableTypes: TableType[] }) {
  ...
  const handlePlaceAll = () => {
    ...
    const isOccupied = (x: number, y: number, allTables: {x: number, y: number}[]) => {
    ...
    // Masa türlerini ayır
    const vipTypes = tableTypes.filter((t) => t.name.toLowerCase().includes('vip'));
    const premiumTypes = tableTypes.filter((t) => 
      t.name.toLowerCase().includes('premium') || t.name.toLowerCase().includes('özel')
    );
    const standardTypes = tableTypes.filter((t) => 
      t.name.toLowerCase().includes('standart') || t.name.toLowerCase().includes('standard')
    );
    
    let num = maxNum + 1;
    const newTables: {x: number, y: number, typeId: string, typeName: string, capacity: number, color: string, shape: string}[] = [];
    ...
    const allTypesToPlace: {type: TableType, count: number, priority: number}[] = [];
    ...
    allTypesToPlace.filter(t => t.priority === 1).forEach(({ type, count }) => {
      let placed = 0;
    ...
    allTypesToPlace.filter(t => t.priority === 2).forEach(({ type, count }) => {
      let placed = 0;
    ...
    allTypesToPlace.filter(t => t.priority === 3).forEach(({ type, count }) => {
      let placed = 0;
      ...
      for (let row = 0; placed < count && row < 30; row++) {
        ...
        for (let col = 0; placed < count && col < 10; col++) {
          ...
          // Sahne alanına girme
          if (y < stageBottom + 20 && x > stageLeftEdge - 60 && x < stageRightEdge + 60) continue;
          // VIP alanına girme
          if (y >= stageBottom && y < sysKontrolY - 60 && x > centerX - 100 && x < centerX + 100) continue;
    ...
    setCounts({});
    console.log(`${newTables.length} masa yerleştirildi`);
  };
  ...
  return (
    <div className="p-3 bg-blue-600/20 rounded-lg border border-blue-600">
      <p className="text-sm text-blue-400 mb-3 font-medium">Hızlı Toplu Ekleme</p>
      <div className="space-y-3 mb-3">
        {tableTypes.map((type) => (
          <div key={type.id} className="flex items-center gap-3">
            ...
            />
            <span className="text-white text-sm flex-1">{type.name} ({type.capacity} kişi)</span>
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                onChange={(e) => setCount(type.id, parseInt(e.target.value) || 0)}
      ...
      {hasNonLocaTables && (
        <div className="mb-4 p-3 bg-green-600/20 rounded-lg border border-green-600">
          <div className="flex items-center justify-between">
            <span className="text-green-400 text-sm font-medium">Toplam Kapasite</span>
            <span className="text-white font-bold text-lg">{totalCapacity} kişi</span>
          </div>
